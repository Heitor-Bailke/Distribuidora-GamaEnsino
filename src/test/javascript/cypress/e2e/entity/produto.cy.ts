...
describe('Produto e2e test', () => {
  ...
  const produtoSample = { nome: 'cruel', valor: 19493.93, quantidade: 4416 };
        ...
        cy.authenticatedRequest({
          ...
          cy.intercept(
            ...
            },
            {
              statusCode: 200,
              headers: {
                link: '<http://localhost/api/produtos?page=0&size=20>; rel="last",<http://localhost/api/produtos?page=0&size=20>; rel="first"',
              },
              body: [produto],
            },
  ...
  describe('new Produto page', () => {
    ...
    it('should create an instance of Produto', () => {
      ...
      cy.get(`[data-cy="valor"]`).type('9386.18');
      cy.get(`[data-cy="valor"]`).should('have.value', '9386.18');

      cy.get(`[data-cy="quantidade"]`).type('7522');
      cy.get(`[data-cy="quantidade"]`).should('have.value', '7522');
